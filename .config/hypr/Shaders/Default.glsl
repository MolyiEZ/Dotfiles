...
out vec4 fragColor;  // <-- declare globally, before main()

const vec3 VIB_RGB_BALANCE = vec3(1.0, 1.0, 1.0);
const float VIB_VIBRANCE = 1.5;
...
void main() {
    ...
    vec3 p_col = (sign(VIB_coeffVibrance) * color_saturation - 1.0)
                 * VIB_coeffVibrance + 1.0;

    pixColor.r = mix(luma, color.r, p_col.r);
    pixColor.g = mix(luma, color.g, p_col.g);
    pixColor.b = mix(luma, color.b, p_col.b);

    fragColor = pixColor;  // assign inside main
}
