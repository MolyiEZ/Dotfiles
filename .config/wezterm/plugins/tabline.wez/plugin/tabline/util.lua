...
local function make_has_icon(process_to_icon)
	return function(arg)
		if not arg then
			return false
		end
		local name = basename(arg):lower()
		return process_to_icon[name] ~= nil
	end
end

function M.pick_argv_with_icon(proc_info, process_to_icon)
	if not proc_info then
		return nil
	end

	local root_argv = proc_info.argv and proc_info.argv[1] or nil
	local has_icon = make_has_icon(process_to_icon)
...
function M.space(s, space, trailing_space)
...
function M.constant_width(prev, next)
