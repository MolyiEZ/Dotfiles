...
return {
	default_opts = {
		process_to_icon = {
			...
			["lf"] = { wezterm.nerdfonts.cod_folder, color = { fg = colors.brights[4] or nil } },
	...
	update = function(tab, opts)
		...
		if active_pane and active_pane.foreground_process_name then
			foreground_process_name = active_pane.foreground_process_name:match("([^/\\]+)[/\\]?$")
		end

		name = tab_title == "" and foreground_process_name or tab_title

		if opts.icons_enabled then
			util.overwrite_icon(
				opts,
				opts.process_to_icon[name]
					or opts.process_to_icon[foreground_process_name]
